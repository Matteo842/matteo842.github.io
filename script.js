...
function generateCodeBackground() {
    ...
    const codeSnippets = [
        ...
        '    # guest offset != host file offset',
        '    def read_at(self, guest_off, size):',
        '        l1, l2 = self.map_l1_l2(guest_off)',
        '        return self.read_cluster(l2)',
        '',
        'def restore_xbsv_v7(backup, live_hdd):',
        '    path = decide_restore_path(live_hdd)',
        '    if path == "remap":',
        '        plan = build_remap_plan(backup, live_hdd)',
        '        return apply_remap(plan)  # FATX free clusters',
        '    return overwrite_or_allocate(backup)  # envelopes',
        '',
        '# XBSV v7: surgical segments + QCOW2 envelopes',
        'SECTOR_SIZE = 512',
        'CLUSTER_SHIFT = 5',
    ];
